...
def radar_data_small():
    return {"i": np.array([1, 2, 3, 4, 5]), "q": np.array([1, 2, 3, 4, 5])}
...
def test_calculate_power_overflow(radar_data_small, radar_data_large_numbers):
    large_numbers_power = RadarPreprocessor.calculate_power(
        radar_data_large_numbers["i"], radar_data_large_numbers["q"]
    )
